...
##### STEP 3: 
Open Octave and head to the song synced lyrics editor.
##### STEP 4: 
...
##### STEP 4: 
Now paste the LRC files to the following path : /sdcard/octave/lyrics/
...
The Octave in-built equalizer was removed updates ago so the only equalizer you will have by your OEM or android which aren't made by us and have no control over them. So you can report those issues to your OEM so that they can provide a fix in next updates.
...
Settings -> Personalize -> Fullscreen controls -> Enable (this will only be visible when songs are playing from Octave).
...
Octave uses the native media player that comes with your Android phone, so as long as a file type is supported by your phone, it's supported by Octave.


#### Q: Why is my device slowing down when I'm using the app?
Octave is image intensive, it keeps images in the cache for quick loading.


#### Q: The title "Octave" is showing on the top of the app, how can i fix this? 
...
• Then open Octave app and manually scan your music from your storage. 

• Reboot the device to refresh media store (Not sure if this is necessary)

NOTE: Don't panic when you will open Octave and see "Zero" songs there in the library. It's because you cleared Media Store which is responsible for recognising files on your device.
...
In your built-in music player, there should be an option to save those playlist as file. Save them and import from file manager by opening it into Octave.

Note that those playlist must be of your offline music only since Octave is offline music player not an online music player. So if your playlist are of online music, it can't be opened on other offline players nor can be exported


#### Q: How to restore/import playlist:
Octave will automatically detect any playlist file when that playlist file is stored in InternalStorage/Playlist. However, if it doesn't, just open "file manager" and open that playlist file with Octave.
