...
object MusicPlayerRemote : KoinComponent {
    ...
    val songProgressTime: String?
        get() = if (musicService != null) {
            musicService!!.songProgressTime
        } else ""

    val songDurationTime: String?
        get() = if (musicService != null) {
            musicService!!.songDurationTime
        } else ""
    ...
    fun playNext(song: Song): Boolean {
        if (musicService != null) {
            ...
            Toast.makeText(
                musicService,
                musicService!!.resources.getString(code.name.score.octave.R.string.added_title_to_playing_queue),
                Toast.LENGTH_SHORT
            ).show()
            return true
        }
    ...
    fun playNext(songs: List<Song>): Boolean {
        if (musicService != null) {
            ...
            val toast =
                if (songs.size == 1) musicService!!.resources.getString(code.name.score.octave.R.string.added_title_to_playing_queue) else musicService!!.resources.getString(
                    code.name.score.octave.R.string.added_x_titles_to_playing_queue,
                    songs.size
                )
