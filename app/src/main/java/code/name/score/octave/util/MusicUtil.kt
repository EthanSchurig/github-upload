...
import code.name.score.octave.db.toSongEntity
import code.name.score.octave.extensions.getLong
import code.name.score.octave.helper.MusicPlayerRemote.removeFromQueue
...
import code.name.score.octave.model.lyrics.AbsSynchronizedLyrics
import code.name.score.octave.repository.RealPlaylistRepository
import code.name.score.octave.repository.RealSongRepository
import code.name.score.octave.repository.Repository
...
object MusicUtil : KoinComponent {
    ...
    fun getReadableDurationString(songDurationMillis: Long): String? {
        ...
        return if (minutes < 60) {
            if (seconds < 10){
                val timeM = "$minutes:0$seconds"
                timeM
            } else {
                val timeM = "$minutes:$seconds"
                timeM
            }
//            String.format(
//                    Locale.getDefault(),
...
//            )
            ...
            if (minutes < 10) {
                if (seconds < 10) {
                    val timeH = "$hours:0$minutes:0$seconds"
                    timeH
                } else {
                    val timeH = "$hours:0$minutes:$seconds"
                    timeH
                }
            } else{
                if (seconds < 10) {
                    val timeH = "$hours:$minutes:0$seconds"
                    timeH
                } else {
                    val timeH = "$hours:$minutes:$seconds"
                    timeH
                }
            }
//            String.format(
